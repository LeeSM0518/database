...
```sql
education=> select * from department;
  dept_code  |    dept_name     |  dept_loc  
-------------+------------------+------------
 D001        | 배우             | 서울특별시
 D002        | 뮤지컬배우       | 서울특별시
 D003        | 가수(솔로)       | 서울특별시
...
education=> select * from drama;
 drm_code |  drm_name   | drm_prd | drm_brd | drm_opdate 
----------+-------------+---------+---------+------------
 DRM01    | 왕의 게임   | TG      | SBC     | 2013-01-01
 DRM02    | 아이러시    | SN      | KBC     | 2013-01-01
 DRM03    | 야킹        | TG      | SBC     | 2013-02-01
...
education=> select * from emp_role;
 emp_rcode | emp_rname  
-----------+------------
 R001      | 엔터테이너
 R002      | 국장
...
education=> select * from employee;
 emp_code | emp_name | emp_mgt | emp_sal | emp_rcode 
----------+----------+---------+---------+-----------
 E001     | 김민훈   | E202    |    5500 | R001
 E002     | 손지민   | E201    |    4500 | R001
 E003     | 이순신   | E203    |    9500 | R001
...
education=> select * from movie;
 mov_code |      mov_name      | mov_mpaa | mov_pddate | mov_opdate 
----------+--------------------+----------+------------+------------
 MOV01    | 모래가 흐르는 바다 | A        | 2013-01-01 | 2013-01-01
 MOV02    | 프랜드             | 18       | 2013-01-15 | 2013-01-15
 MOV03    | 5급 공무원         | 15       | 2013-02-01 | 2013-02-01
...
education=> select * from music;
 msc_code |     msc_name     |  msc_date  | msc_price | msc_csf 
----------+------------------+------------+-----------+---------
 MSC01    | 소년시대 2013    | 2013-01-01 |      8000 | 싱글
 MSC02    | 하이퍼주니어 4집 | 2013-01-05 |     15500 | 정규
 MSC03    | 이승모 연인      | 2013-01-31 |      7000 | 싱글
...
education=> select * from part_drama;
 pd_drm_code | pd_emp_code | pd_emp_role | pd_emp_fee 
-------------+-------------+-------------+------------
 DRM01       |             |             |           
 DRM02       | E003        | 주연        |      13500
 DRM02       | E001        | 조연        |       7500
...
education=> select * from part_movie;
 pm_mov_code | emp_code | pm_emp_role | pm_mov_fee 
-------------+----------+-------------+------------
 MOV01       |          |             |           
 MOV02       |          |             |           
 MOV03       | E003     | 주연        |      13500
...
education=> select * from part_music;
 pm_msc_code | pm_emp_code | pm_emp_role | pm_emp_fee 
-------------+-------------+-------------+------------
 MSC01       | E008        | 메인        |       8500
 MSC02       |             |             |           
 MSC03       | E007        | 피처링      |       1500
...
education=> select * from rel_department;
 rd_emp_code | rd_dept_code 
-------------+--------------
 E001        | {D001,D002}
 E002        | D001
 E003        | {D001,D003}
